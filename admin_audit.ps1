...
$params = @{
    Left = $adminusers
    Right = $auditlogs
    Where = {$args[0].username -eq $args[1].user}
    LeftProperties = "username"
    RightProperties = "user","action_time"
    Type = 'AllInLeft'
}
...
ForEach-Object {
    $active= 'False'
    if ($null -ne $_.action_time) { $active = 'True' }

    Add-Member -InputObject $_ -MemberType NoteProperty -Name Active -Value $active -PassThru
}

$Join | sort-object -Property username -Unique | Export-Csv .\device42_administrators.csv -NoTypeInformation